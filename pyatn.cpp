...
int ManhattanDistance(FieldsClass &f1, const FieldsClass &f2, const std::string &step);
void heuristicMethod(FieldsClass &f1, const FieldsClass &f2, const std::string &step);
...
void heuristicMethod(FieldsClass &f1, const FieldsClass &f2, const std::string &step)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if(!flag && (step != "right") && (f1.m_empty_field.y > 0))
            {
                LeftDist = ManhattanDistance(f1,f2,"left");
                ValuesVec.push_back(LeftDist);
            }
            if(!flag && (step != "left") && (f1.m_empty_field.y < (f1.getColumn() - 1)))
            {
                RightDist = ManhattanDistance(f1,f2, "right");
                ValuesVec.push_back(RightDist);
            }
            if(!flag && (step != "bottom") && (f1.m_empty_field.x > 0))
            {
                TopDist = ManhattanDistance(f1,f2,"top");
                ValuesVec.push_back(TopDist);
            }
            if(!flag && (step != "top") && (f1.m_empty_field.x < (f1.getRow() - 1)))
            {
                BottomDist = ManhattanDistance(f1,f2,"bottom");
...
int main() {
    ...
    cout<<endFields;
    cout<<"\nheuristic:"<<endl;
    heuristicMethod(startFields,endFields, "start");
    cout<<"steps:"<<endl;
